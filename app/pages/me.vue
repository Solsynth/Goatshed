<template>
  <main class="page-shell py-8">
    <div class="mx-auto max-w-4xl">
      <div v-if="!auth.authenticated.value" class="rounded-box border border-base-300 bg-base-100 p-8 text-center">
        <User class="mx-auto h-12 w-12 text-base-content/50" />
        <h1 class="mt-4 text-xl font-bold">未登录</h1>
        <p class="mt-2 text-sm text-base-content/60">登录以查看你的个人资料。</p>
      ...
      <template v-else-if="auth.user.value">
        <section class="relative overflow-hidden rounded-2xl border border-base-300 bg-base-100">
          ...
          <div class="mx-auto -mt-16 flex flex-col gap-3 px-4 pb-4 sm:-mt-16 sm:flex-row sm:items-end">
            <div class="mb-12 shrink-0">
              <div v-if="avatarUrl" class="avatar">
                <div class="h-20 w-20 rounded-full ring ring-base-300 ring-offset-2 ring-offset-base-100 sm:h-24 sm:w-24">
                  <img :src="avatarUrl" :alt="displayName">
                </div>
              </div>
              <div v-else class="avatar avatar-placeholder">
                <div class="h-20 w-24 rounded-full bg-primary text-primary-content ring ring-base-300 ring-offset-2 ring-offset-base-100 sm:h-24 sm:w-24">
            ...
            <div class="min-w-0 flex-1 max-lg:-mt-10 lg:pt-20">
              <div class="flex flex-wrap items-center gap-2">
                <h1 class="truncate text-xl font-black sm:text-2xl">{{ displayName }}</h1>
                <span v-if="account?.profile?.verification" class="badge badge-primary gap-1">
                  <ShieldCheck class="h-3 w-3" />
                  {{ account.profile.verification.title || "已认证" }}
                </span>
              </div>
              <p class="truncate text-sm text-base-content/60">@{{ auth.user.value.username || auth.user.value.name }}</p>
            </div>
          </div>
        </section>
          ...
          <div class="rounded-box border border-base-300 bg-base-100">
            <div class="p-2">
              <div class="flex items-center gap-3 rounded-xl p-3">
                ...
                <span class="text-sm">{{ account?.profile?.email || "-" }}</span>
              </div>
              <div class="flex items-center gap-3 rounded-xl p-3">
              ...
              <div class="flex items-center gap-3 rounded-xl p-3">
                ...
                <span class="text-sm">{{ account?.profile?.location || "-" }}</span>
              </div>
              <div class="flex items-center gap-3 rounded-xl p-3">
                <Clock class="h-5 w-5 text-base-content/70" />
                <span class="flex-1 text-sm text-base-content/60">时区</span>
                <span class="text-sm">{{ account?.profile?.timeZone || "-" }}</span>
              </div>
            </div>
          </div>
...
import { User, ShieldCheck, ChevronRight, Mail, Globe, MapPin, MapPinned, Clock, LogOut, Loader2 } from "lucide-vue-next";
