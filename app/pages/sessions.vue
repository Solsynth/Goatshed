    ...
    <main class="page-shell py-8">
        <div class="mx-auto max-w-3xl">
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-base-content/60">消耗陪玩票加入场次，和小羊一起玩！</p>
            </div>
            ...
            <div v-if="sessions.length > 0" class="space-y-4">
                ...
                >
                    <div class="flex items-start justify-between">
                        ...
                        <span class="badge badge-info">{{ s.ticketCost }} 张票</span>
                    </div>
                    <div class="mt-3 flex items-center justify-between text-sm text-base-content/50">
                        <span>{{ s.participantCount }} 人已加入</span>
                        <button
                            class="btn btn-primary btn-sm"
                            :disabled="joiningId === s.id || ticketBalance.available < s.ticketCost"
                            :title="ticketBalance.available < s.ticketCost ? '票数不足' : ''"
                            @click="joinSession(s)"
                        >
                            <Loader2 v-if="joiningId === s.id" class="h-4 w-4 animate-spin" />
                            {{ ticketBalance.available < s.ticketCost ? '票数不足' : '加入' }}
                        </button>
                    </div>
                </div>
            </div>
...
async function fetchSessions() {
    ...
    try {
        const [data, tickets] = await Promise.all([
            $fetch("/api/sessions") as Promise<Session[]>,
...
async function joinSession(s: Session) {
    ...
    try {
        await $fetch(`/api/sessions/${s.id}/join`, { method: "POST" });
        joinResult.value = true;
        joinMessage.value = `成功加入「${s.name}」！消耗了 ${s.ticketCost} 张陪玩票。`;
        resultDialog.value?.showModal();
        ticketBalance.value.available -= s.ticketCost;
        ticketBalance.value.used += s.ticketCost;
        await fetchSessions();
    } catch (e: any) {
